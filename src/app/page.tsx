...
export default function Home() {
  ...
  const approve = () => {
    ...
    setEditedSegments(null); setEditedFlows(null); setEditedContent(null); setEditedAttribution(null); setThumb(undefined);
  };
  const revise = () => { if (feedback) { runStream("/api/session/stage", { sessionId: session.id, action: "revise", feedback }); setFeedback(""); } };

  useEffect(() => { setEditedSegments(null); setEditedFlows(null); setEditedContent(null); setEditedAttribution(null); setThumb(undefined); }, [session?.stageIndex, session?.id]);
  ...
  return (
    <main style={{ maxWidth: 1040, margin: "0 auto", padding: "34px 20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        ...
        {session && <div style={{ textAlign: "right", fontSize: 12, color: C.t3 }}>
          <div>session cost</div><div style={{ fontSize: 18, fontWeight: 600, color: C.purple }}>${cost(session).toFixed(6)}</div>
          <div>{session.events?.length || 0} model calls</div></div>}
      </div>
        ...
        <div style={{ display: "flex", gap: 18, marginTop: 18, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 300 }}>
            ...
            {!session.done && (() => {
              ...
              if (curKey === "content") return wrap("Content — edit variants, add A/B", <ContentEditor initial={(session.outputs?.content?.offers) || []} onChange={setEditedContent} />);
            ...
            {session.done && (
              <div style={card}><p style={{ ...label, color: C.green }}>Strategy brief ready</p>
                <p style={{ marginTop: 0 }}>All six stages approved. Download the formatted brief:</p>
      ...
      {!session && history.length > 0 && (
        ...
        <div style={card}><p style={label}>Monitoring — {username}'s sessions ({history.length})</p>
          {history.map((s) => (<div key={s.id} style={{ borderTop: `1px solid ${C.line}`, padding: "8px 0", fontSize: 13 }}>
            <b>{s.goalText}</b> <span style={{ color: C.t3 }}>· {s.vertical} · {s.done ? "complete" : `stage ${s.stageIndex + 1}/6`} · ${cost(s).toFixed(6)}</span></div>))}
        </div>
        </>
      )}
    </main>
