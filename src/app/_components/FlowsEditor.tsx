...
function FlowCard({ flow, content, segmentNames, onChange, onDelete }:
  ...
  const audienceOpts = Array.from(new Set([flow.audience, ...segmentNames].filter(Boolean)));
  return (
    <div style={{ border: `1.5px solid ${C.line}`, borderRadius: 10, padding: 12, marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8 }}>
        ...
        <select value={flow.audience} onChange={(e) => upd({ audience: e.target.value })} style={{ ...inp, flex: 1 }}>
          <option value="">— select segment —</option>
          {audienceOpts.map((a) => <option key={a} value={a}>{a}</option>)}
        </select>
      </div>
