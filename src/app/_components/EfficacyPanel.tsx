...
export default function EfficacyPanel({ history }: { history: any[] }) {
  const agg: Record<string, { n: number; edited: number; up: number; down: number; rev: number; tokens: number; cost: number }> = {};
  for (const s of history) {
    for (const [k, r] of Object.entries((s.ratings || {}) as Record<string, any>)) {
      agg[k] ??= { n: 0, edited: 0, up: 0, down: 0, rev: 0, tokens: 0, cost: 0 };
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid rgba(0,0,0,.1)", borderRadius: 14, padding: 20, marginBottom: 16 }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
        <thead><tr>{["Component", "Approved", "Edited %", "👍/👎", "Avg revisions", "Tokens", "Cost"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
        <tbody>{rows.map(([k, a]) => (
          <tr key={k}>
