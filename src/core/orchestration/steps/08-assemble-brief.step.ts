...
import { computeSchedule } from "@core/domain/plan/schedule";
...
export async function assembleBrief(session: Session): Promise<Buffer> {
  ...
  if (o.plan) {
    const sched = computeSchedule(o.plan);
    children.push(h("Project plan"));
    children.push(p(`Timeline: ${sched.projectStart} → ${sched.projectEnd}`));
    const idName = new Map(sched.tasks.map((t) => [t.id as string, t.name]));
    // Display order: each top-level task followed by its children.
    const tops = sched.tasks.filter((t) => !t.parentId);
    const ordered = tops.flatMap((tp) => [tp, ...sched.tasks.filter((c) => c.parentId === tp.id)]);
    const rows = [
      new TableRow({ children: ["Task", "Owner", "Days", "Start", "End", "Depends on"].map((t) => new TableCell({ children: [bold(t)] })) }),
      ...ordered.map((t) => new TableRow({ children: [
        (t.parentId ? "    ↳ " : "") + t.name,
        t.owner,
        t.isParent ? "—" : String(t.durationDays),
        t.start, t.end,
        (t.deps || []).map((d) => idName.get(d) || "").filter(Boolean).join("; "),
